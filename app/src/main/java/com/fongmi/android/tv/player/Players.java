...
public class Players implements Player.Listener, IMediaPlayer.Listener, AnalyticsListener, ParseCallback, DrawHandler.Callback {
    ...
    private void setupExo(PlayerView view) {
        exoPlayer = new ExoPlayer.Builder(App.get())
                .setLoadControl(ExoUtil.buildLoadControl(Setting.getBuffer()))
...
//        exoPlayer.setSeekParameters(SeekParameters.EXACT);
    ...
    public void checkData(Intent data) {
        try {
            ...
            if ("playback_completion".equals(endBy)) seekTo((long) getDuration());
            if ("user".equals(endBy)) seekTo((long) position);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
